...
2. **The TTL is enforced at read time, not cryptographically.** murk refuses an expired grant at every entry point (CLI, MCP, the bindings), and `agent ls` flags it. But age keys can't self-destruct, and old `.murk` versions stay readable in git, so a leaked grant key works against history with raw age until `agent revoke` plus rotation.

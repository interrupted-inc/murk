...
**Library bindings (Python/Node) and the FFI boundary.** The bindings hand plaintext to a host runtime, and murk's zeroization cannot follow it there. On a **read** (`get`/`export`), the decrypted value is copied out of `Zeroizing<String>` into a native host string — a Python `str`, a V8 `String` — that the host's garbage collector owns and frees on its own schedule; murk never gets to wipe it. On a **write** (Node `vault.add` / the write path), the exposure is sharper still: the caller constructs the plaintext as a host string *before* calling murk, so it already lives in GC-managed memory the moment it exists, is copied across the FFI boundary into a Rust `String`, and is zeroized on neither side. A write means the plaintext originated in your process, not in murk's decryption path, and lingers under the GC's control. This is inherent to moving secrets across a language boundary; build write values as late as possible, pass them straight in, and keep no long-lived references. The bindings otherwise inherit every guarantee enforced in the library load/save path (agent policy, grant TTLs, signer pinning, the keyed MAC) — the leak is memory hygiene, not access control.
